...
export const getSessionList = async (req, res) => {
  try {
    ...
    // 2. 查询当前用户所有的会话列表
    const [rows] = await db.query(
      `select * from chat_session_view where ( user1_id = ? and deleted_by_user1 = 0 ) or ( user2_id = ? and deleted_by_user2 = 0 ) order by ifnull(last_message, last_time) desc`,
      [user_id, user_id],
    );
...
export const getMessage = async (req, res) => {
  try {
    ...
    // 2. 查询会话中的所有消息
    const [rows] = await db.query(
      `select * from chat_message_full_view where session_id = ? order by created_at asc`,
      [session_id],
    );
...
export const senderMessageHandler = async (req, res) => {
  try {
    ...
    if (!session) return res.cc("会话不存在");
    ...
    const receiver_id =
      session.user1_id === sender_id ? session.user2_id : session.user1_id;
    ...
    const receiver = onlineUsers[receiver_id];
    ...
    const sender = onlineUsers[sender_id];
...
export const deleteSession = async (req, res) => {
  try {
    ...
    if (session.user1_id === user_id) {
      await db.query(
        `update chat_sessions set deleted_by_user1 = 1 where id = ?`,
        [session_id],
      );
    } else {
      await db.query(
        `update chat_sessions set deleted_by_user2 = 1 where id = ?`,
        [session_id],
      );
    }
