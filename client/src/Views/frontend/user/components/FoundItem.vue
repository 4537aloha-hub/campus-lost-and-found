...
// 2. 查询found物品列表
import { getItemListByUserIdFound } from '@/api/user'
...
// 定义loading状态
const loading = computed(() => userStore.token && !itemFound.value)
...
const getFoundItemList = async () => {
  ...
  const res = await getItemListByUserIdFound(params.value)
...
<template>
  <page-container title="我发布的" v-loading="loading">
  <template #button>
  ...
  <!-- 表格区 -->
   <template #table>
    ...
    <el-table v-else :data="itemFound.list">
